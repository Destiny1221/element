/**
 * 根据组件配置文件components.json结合字符串模版，自动生成/src/index.js文件，避免每次新增组件时手动在src/index.js文件中导入导出组件
 * 生成src/index.js 文件
 * 1、自动导入组件库所有组件
 * 2、定义全量注册组件库组件的install方法
 * 3、导出版本、install、各个组件
 */
...
/**
 * 模板库，用于给字符串中的变量替换
 * 例子: var str = 'interpolation is nice {{myVal}}'
 * require('json-templater/string')(str,{
 *    myval:'value'
 * })
